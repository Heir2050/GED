<?php
// avec solution a la fin bidons de chatgpt
    ...
    public function index($dossier_id = null)
    {
                ...
                foreach ($file['files']['name'] as $index => $fileName) {
                    if ($file['files']['error'][$index] === UPLOAD_ERR_OK && !empty($fileName)) {
                        ...
                        if (move_uploaded_file($file['files']['tmp_name'][$index], $filePath)) {
                            $documentData = [
                                'nom' => $originalName,
                                'nom_stockage' => $dossierNameClean . '/' . $storageName,
                            ...
                            if ($document->insert($documentData)) {
                                message("✅ Fichier enregistré : " . $originalName);
                                $uploadedFiles++;
                            } else {
                                message("❌ Erreur BD pour : " . $originalName);
                            }
                        } else {
                            message("❌ Erreur upload pour : " . $originalName);
                        }
                    }
                }
        ...
        if ($dossier_id) {
            ...
            foreach ($data['dossiers'] as $d) {
                $data['counts'][$d->id] = $document->query("SELECT COUNT(*) as count FROM documents WHERE id = :id", 
                    ['id' => $d->id])[0]->count;
            }
        }
