<?php 
    ...
    $page = 'dashboard'; // par défaut

    if (strpos($current_uri, '/document/archives') !== false) {
        $page = 'archives';
    } elseif (strpos($current_uri, '/document') !== false) {
        $page = 'document';
    } elseif (strpos($current_uri, '/users') !== false) {
        $page = 'users';
    } elseif (strpos($current_uri, '/historique') !== false) {
        $page = 'historique';
    }
    // Ajoutez d'autres conditions selon vos routes
?>
    ...
    <div :class="sidebarToggle ? 'justify-center' : 'justify-between'" class="flex items-center gap-2 pt-8 sidebar-header pb-7">
        <a href="<?= ROOT ?>" class="flex items-center gap-2">
            <span class="logo" :class="sidebarToggle ? 'hidden' : ''">
                ...
                <img class="hidden dark:block" src="<?= ROOT ?>/assets/images/logo.png" alt="Logo" width="120px"/>
            </span>

            <img class="logo-icon" :class="sidebarToggle ? 'lg:block' : 'hidden'" src="<?= ROOT ?>/assets/images/logo.png" alt="Logo" />
        </a>
    </div>
    <!-- SIDEBAR HEADER -->
        ...
        <nav x-data="{selected: $persist('Dashboard')}">
            ...
            <div>
                ...
                <ul class="flex flex-col gap-4 mb-6">
                    <!-- Home -->
                    <li>
                        <a href="<?= ROOT ?>" class="menu-item group <?= $page === 'dashboard' ? 'menu-item-active bg-gray-100 dark:bg-gray-800' : 'menu-item-inactive' ?>">
                            <svg
                    ...
                    <?php if ($ses->is_logged_in() && $ses->user('role') == 'ADMIN') : ?>
                        <li>
                            <a href="<?= ROOT ?>/users" class="menu-item group <?= $page === 'users' ? 'menu-item-active bg-gray-100 dark:bg-gray-800' : 'menu-item-inactive' ?>">
                    ...
                    <li>
                        <a href="<?= ROOT ?>/document" onclick="markAllNotificationsAsRead()" class="menu-item group <?= $page === 'document' ? 'menu-item-active bg-gray-100 dark:bg-gray-800' : 'menu-item-inactive' ?>">
                            <svg viewBox="0 0 64 64" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><g id="Outline"><g data-name="Outline" id="Outline-2">
                                ...
                                <circle cx="6" cy="46" r="1"/></g></g>
                            </svg>
                            <span class="menu-item-text" :class="sidebarToggle ? 'lg:hidden' : ''">
                            Documents
                            <?php if (isset($notifCount) > 0): ?>
                                <span :class="!notifying ? 'hidden' : 'flex'" class="absolute right-0 z-1 rounded-full bg-brand-500" style="height: 1.2rem;width: 1.2rem; display: flex; align-items: center; justify-content: center; color: white; font-size: .8rem;right: 5px;top: 12px;">
                                    <?= $notifCount ?>
                                </span>
                            <?php endif; ?>
                            </span>
                    ...
                    <li>
                        <a href="<?= ROOT ?>/document/archives" class="menu-item group <?= $page === 'archives' ? 'menu-item-active bg-gray-100 dark:bg-gray-800' : 'menu-item-inactive' ?>">
                            <svg height="24px" width="24px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 491.52 491.52" xml:space="preserve"><g><g>
                                <path d="M0,37.334v417.095h120.737V37.334H0z M33.361,75.287h54.015v125.488H33.361V75.287z M60.369,416.475 c-14.916,0-27.008-12.091-27.008-27.008c0-14.915,12.091-27.007,27.008-27.007s27.008,12.091,27.008,27.007C87.376,404.384,75.285,416.475,60.369,416.475z"/></g></g>
                                <g><g><path d="M383.618,28.684l-116.424,31.18v-22.53H146.372v417.095h120.823V61.006l107.617,401.83L491.52,431.58L383.618,28.684z
                    ...
                    <li>
                        <a href="<?= ROOT ?>/historique" class="menu-item group <?= $page === 'historique' ? 'menu-item-active bg-gray-100 dark:bg-gray-800' : 'menu-item-inactive' ?>">
